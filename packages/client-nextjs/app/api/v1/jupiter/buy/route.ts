...
export async function POST(request: Request) {
    try {
        console.log("[API ROUTE] Incoming request to Jupiter API Proxy (Buy)");

        const body = await request.json();
        console.log(`[API ROUTE] Parsed request body:`, body);

        let { ca, amount, slippage } = body;

        if (!ca || !amount || slippage === undefined) {
        ...
        const apiEndpoint = `${API_URL}/api/v1/jupiter/buy`;
        ...
        console.error(`[API ROUTE] Error in Buy API route:`, error);
